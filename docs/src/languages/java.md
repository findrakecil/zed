...
Java language support in Zed is available through [Java extension](https://github.com/zed-extensions/java).
...
There is a fairly straightforward fix that you can apply to make it work on Windows by supplying your own task scripts. Please see [this issue](https://github.com/zed-extensions/java/issues/94) for information on how to do that and read the [Tasks](https://zed.dev/docs/tasks) section in Zeds documentation for more information.
...
Below is an example `settings.json` that would pass on the example configuration from the above wiki page to JDTLS:
