...
If you would like to use a specific Dart binary or use Dart via FVM you can specify the `dart` binary in your Zed `settings.json` file:
...
Dart uses a very conservative (80) maximum line length by default. If you would like to permit a longer line length when auto-formatting, add the following to your Zed `settings.json`:
